...
const Hero = () => {
const {keyword,setKeyword,setApiData,setLoader,loader} = useContext(Context);
...
useEffect(()=>{
 ...
 searchHandler(setApiData,setLoader,state);
  ...
  return (
    ...
    <Box paddingY={"5"} paddingX={[8,8,8,20]}  float={""} >
      <HStack justifyContent={["space-between","space-between","space-between","space-around"]}>
        <Link  href="/" >    <Heading
          width={"auto"}
          size={"lg"}
          fontWeight={"light"}
        ...
        >
          <GoSearch
            style={{ top: "13px",left: "10px",position: "absolute",zIndex: 1,fontSize: "20px",color: "gray"}}
          />
          <Input
            type="text"
            value={keyword}
        ...
        <HStack  display={["none","none","none","flex"]} >
          <Text fontSize={"15px"} fontWeight={"bold"}>Dark Mode</Text> 
 ...
 {
  (!loader&&keyword?true:false)&&<Box>
    <Heading size={"2xl"} textColor={"gray.600"}  padding={[10,10,10,20]} >{keyword}</Heading>
  </Box>
 }
    {
      !keyword&&   <Box
    ...
    >
      ...
      <Box
        ...
        left={0}
      >
        <VStack justifyContent={"center"} height={"100%"}  textAlign={"center"}>
